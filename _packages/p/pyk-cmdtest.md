...
author: pyk
author_github: pyk
repository: https://github.com/pyk/cmdtest
keywords:
  - cli
  - testing
date: 2026-06-23
category: tooling
updated_at: 2026-06-23T08:11:05+00:00
last_sync: 2026-06-23T08:11:05Z
package_kind: hybrid
has_library: true
has_binary: true
...
permalink: /packages/pyk/cmdtest/
---
...
1. Fetch the latest release:
    ...
    ```shell
    zig fetch --save=cmdtest https://github.com/pyk/cmdtest/archive/v0.3.0.tar.gz
    ```
...
- `add(b, options)` registers a CLI test step in `build.zig`. `zig build test`
  will run the tests.
- `run(io, options)` spawns a child process, captures stdout and stderr (up to a
...
test "cat" {
    ...
    const input = "a\nb\n";
    var result = try cmdtest.run(testing.io, .{ .argv = argv, .stdin = input });
    defer result.deinit();
...
var result = try cmdtest.run(testing.io, .{
  .argv = argv,
  .stdin = payload,
